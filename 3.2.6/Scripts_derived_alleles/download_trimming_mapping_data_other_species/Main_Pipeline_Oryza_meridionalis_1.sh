...
### INFO
genus=$(echo "Oryza")
species=$(echo "meridionalis")
listacc=$(echo "SRR1528444")
...
for j in $listacc; do
    ...
    bash $pathtoscripts/1_mapping.sh $j $pathtodata/$j/Trimmomatic *_1_cleaned.fastq.gz *_2_cleaned.fastq.gz *_1_cleaned_unpaired.fastq.gz *_2_cleaned.unpaired.fastq.gz $refile 8
    # CMD bash 2_snpindel_callingGVCF.sh IDname Reference_Genome output_directory Number_of_CPU_to_use 
    bash $pathtoscripts/2_snpindel_callingGVCF.sh $j $refile $pathtodata/$j 8
    cd ..
done
