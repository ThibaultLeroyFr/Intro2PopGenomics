...
module load java
for j in ERR3284869; do
        cd /sandbox/users/tleroy/SessileOak/
        mkdir Trimmomatic
        ...
        for i in $seqlines; do
            acc=$(echo "/sandbox/users/tleroy/SessileOak/ERR2008849/$j/$i")
            outacc=$(echo "/sandbox/users/tleroy/SessileOak/ERR2008849/$j/Trimmomatic/$i")
